...
window.results_documents = new Array();;
...
var keywordModel = function() {
	...
	debug("got " + keyCount + " related keys");
...
var getThumbnail = function(resource_locator) {
	...
	var returnVal = jQuery.mustache(template, {
...
var filterSlice = function(filter) {
	...
	target.html("");
	if (window.results_documents ) {
//		console.log("have documents");
		doc = window.results_documents ;
	...
	if(data) {
		...
		if(data.resumption_token) {
			$.ajax({
			  ...
			  dataType: 'jsonp',
			  jsonp: 'callback',
			  data: { "any_tags": window.keys.val(), "resumption_token":data.resumption_token },
			  success: handleSlice
			});	
		} else {
			parseSliceResults();
...
var parseSliceResults = function() {
	window.cur_related_keys = {};
	//console.log(data);
//	for (key in data) {
//		console.log("key:"+key);
...
//		console.log("have documents");
		doc = window.results_documents;
		debug("got " + doc.length + " result documents");
		for (var i=0; i < doc.length; i++) {
